AVAILABLE_TASK_FUNCTIONS = {
    "create_task": {
        "name": "create_task",
        "description": "The task starts at step 1. The only available steps are: cutting a clip into two (mention clip), moving a clip (mention clip and time at which to place), deleting a clip (mention clip), brightness of a clip (mention clip and describe parameters), applyColorGrading (clip and parameters), convertToGrayscale (mention clip), addBlurEffect (clip), adjustSaturation (clip, parameters), apply fade in (mention clip and duration), apply fade out (mention clip and duration). All times are relative to the start of each clip. Put each use of a tool in a new step. Each step should be a very basic and clear command in terms of which tool to apply to which clip. Consider that every time you cut a clip, a new clip will be created. You should mention which clip you want to make changes to using their relative positioning like 'the third clip'. Each step should only require the editor to call one function.",
        "parameters": {
            "type": "object",
            "properties": {
                "steps": {
...
AVAILABLE_FUNCTIONS = {
    ...
    "applyFadeIn": {
        "name": "applyFadeIn", 
        "description": "Apply a fade in effect for a transition for the specified duration.",
        "parameters": {
            "type": "object",
            "properties": {
                "clipId": {
                    "type": "string",
                    "description": "ID of the clip at the beginning of which we will add the fade in"
                },
                "duration": {
                    "type": "number",
                    "description": "Duration of the fade in"
                }
            },
    ...
    "applyFadeOut": {
        "name": "applyFadeOut", 
        "description": "Apply a fade out effect for a transition for the specified duration.",
        "parameters": {
            "type": "object",
            "properties": {
                "clipId": {
                    "type": "string",
                    "description": "ID of the clip at the end of which we will add the fade out"
                },
                "duration": {
                    "type": "number",
